...
## Mission 029 — Product demonstration boundary

Mission 029 is not a benchmark and generated no benchmark or comparative metrics. It ran one fresh Bright Data collector creation, one collector run, and one approval-gated healing request against public Hacker News evidence. The fresh collector `c_msyo46bp1slx64351` produced 150 structured rows after a provider-observed realtime-to-batch fallback. A reversible AEGIS-only `DEMO_MUTATION` changed one preserved `points` value from `402` to `-1`; the live provider output and external target remained unchanged.

The existing deterministic detector returned L3 evidence. The existing deterministic diagnosis correctly remained `UNKNOWN` / `AMBIGUOUS` because the signal set crossed schema, statistical, and semantic classes. The one healing request failed before candidate creation because the 1,187-character bounded repair prompt exceeded the provider’s reported 1,000-character limit. No retry, approval, candidate, verification, RiskGovernor, CommitGate, commit, benchmark, Gemini call, or NVIDIA benchmark call occurred. The provider failure is preserved in `experiments/mission_029/` and is not relabeled as repair evidence.

## Mission 030 — Provider-transport compatibility validation

Mission 030 is not a benchmark and generated no benchmark or comparative metric. It preserves the immutable Mission 029 `RepairRequest` and derives a deterministic, compact Bright Data transport projection only. The projection has policy version `mission-030-compact-heal-prompt-v1`, SHA-256 `d766547e1991faa67fccfdc58ed2ffe09a2b7cfc8e4a03418243db2cc407d9b5`, and measured length `676`, below the previously observed provider limit of `1,000`.

One authorized Bright Data heal was submitted against the existing Mission 029 collector. It performed zero collector creates and runs, returned terminal provider status `FAILED` with a 500 failure before candidate creation, and had `407029` ms observed latency. AEGIS issued no external retry, approval, verification, risk decision, commit, activation, shipment, Gemini call, NVIDIA benchmark call, or metric calculation. Provider/CLI internal retry diagnostics are recorded as provider behavior, not an AEGIS retry. The terminal result is preserved in `experiments/mission_030/` and does not constitute recovery evidence.
