#!/usr/bin/env python3
"""Provider-free preflight for the Mission 028 clean recovery attempt."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Mission 028 recovery before trial 1")
    parser.add_argument("--root", default=Path(__file__).resolve().parents[1], type=Path)
    parser.add_argument("--output", default="experiments/mission_028_recovery_preflight.json", type=Path)
    args = parser.parse_args()
    root = args.root.resolve()
    gate, config, run_id = recovery_preflight(root)
    if run_id == MISSION_028_INVALIDATED_RUN_ID:
        raise RuntimeError("recovery identity illegally reuses the invalidated Mission 028 run ID")
    payload = {
        "mission": "028",
        "attempt_kind": "RECOVERY",
        "status": "RECOVERY_PREFLIGHT_PASS" if gate.passed else "STOPPED_RECOVERY_PREFLIGHT",
        "notice": "NO RECOVERY TRIAL EXECUTED",
        "invalidated_first_attempt_run_id": MISSION_028_INVALIDATED_RUN_ID,
        "recovery_attempt_id": MISSION_028_RECOVERY_ATTEMPT_ID,
