...
ACTUAL_AEGIS_REVISION = "b79050044be0a6d919eecd5633f72188469022df"
...
def test_historical_mission016_directory_is_present_and_untouched() -> None:
    historical_before = _directory_digest(HISTORICAL_ROOT)
    smoke_before = _directory_digest(CORRECTED_ROOT)
    ...
    assert _directory_digest(HISTORICAL_ROOT) == historical_before
    assert _directory_digest(CORRECTED_ROOT) == smoke_before
...
def test_old_config_is_fail_closed_when_substituted(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert preflight.BENCHMARK_RUN_ROOT.exists() is False
