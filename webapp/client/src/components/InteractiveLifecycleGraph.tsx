  ...
  return <section className={`interactive-lifecycle mode-${state.mode.toLowerCase()}`} aria-label={title}>
    <div className="lifecycle-canvas">
      <svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
        ...
        {state.edges.map((edge) => <line key={edge.id} className={`is-${edge.status.toLowerCase()} edge-${edge.provenance.toLowerCase()} ${state.activeEdgeIds.includes(edge.id) ? "is-active-edge" : ""}`} x1={positions[edge.source].x} y1={positions[edge.source].y} x2={positions[edge.target].x} y2={positions[edge.target].y} />)}
      </svg>
      {state.nodes.map((node) => <button type="button" key={node.id} className={`lifecycle-node node-${node.status.toLowerCase()} ${selected.id === node.id ? "is-selected" : ""}`} style={{ left: `${positions[node.id].x}%`, top: `${positions[node.id].y}%` }} onClick={() => setSelectedId(node.id)} aria-pressed={selected.id === node.id} aria-label={`${node.label}: ${node.status}. ${node.detail}`}><i /><span>{node.label}</span><small>{node.status.replaceAll("_", " ")}</small></button>)}
    </div>
    <div className="lifecycle-inspector" aria-live="polite">
      <div className="lifecycle-inspector-heading"><div><span className="eyebrow">Selected lifecycle node</span><h3>{selected.label}</h3></div><StatusBadge status={selected.status} /></div>
      <p>{selected.detail}</p>
      ...
      {selected.status === "BLOCKED" && <p className="lifecycle-fail-closed"><ExternalLink size={14} /> The commit path is fail-closed until the missing evidence boundary is satisfied.</p>}
    </div>
  </section>;
}
