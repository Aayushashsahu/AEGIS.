...
export type CaseGraph = { case_id: string; mode: "CASE" | "REPLAY" | "CONTROLLED_DEMONSTRATOR" | "EMPTY" | "REAL_PROVIDER" | "REAL_PROVIDER_CAUSAL_BOUNDARY" | "TEST_DOUBLE_CONTROLLED_REPLAY" | "CONFIGURED_CASE"; nodes: Array<{ id: "TARGET" | "COLLECTOR" | "OBSERVATION" | "DETECTION" | "DIAGNOSIS" | "REPAIR" | "CANDIDATE" | "VERIFICATION" | "RISK" | "COMMIT"; label: string; domain_status: string; display_status: "CONFIGURED" | "ACTIVE" | "PENDING" | "ANOMALY" | "VERIFIED" | "QUARANTINED" | "BLOCKED" | "UNAVAILABLE"; provenance: "USER_CONFIGURED" | "NORMALIZED_CASE" | "REAL_PROVIDER" | "REPLAY" | "TEST_DOUBLE" | "CONTROLLED_DEMONSTRATOR" | "AEGIS_DETERMINISTIC"; detail: string; evidenceRefs: string[]; provider?: string }>; edges: Array<{ id: string; source: string; target: string; domain_status: string; display_status: "PENDING" | "ACTIVE" | "BROKEN" | "VERIFIED" | "UNAVAILABLE"; provenance: "USER_CONFIGURED" | "NORMALIZED_CASE" | "REAL_PROVIDER" | "REPLAY" | "TEST_DOUBLE" | "CONTROLLED_DEMONSTRATOR" | "AEGIS_DETERMINISTIC"; evidenceRefs: string[] }>; activeNodeId: "TARGET" | "COLLECTOR" | "OBSERVATION" | "DETECTION" | "DIAGNOSIS" | "REPAIR" | "CANDIDATE" | "VERIFICATION" | "RISK" | "COMMIT"; activeEdgeIds: string[]; currentStage: "TARGET" | "COLLECTOR" | "OBSERVATION" | "DETECTION" | "DIAGNOSIS" | "REPAIR" | "CANDIDATE" | "VERIFICATION" | "RISK" | "COMMIT"; severity: "UNKNOWN" | "HEALTHY" | "ANOMALY" | "QUARANTINE" | "BLOCKED"; provenance: "USER_CONFIGURED" | "NORMALIZED_CASE" | "REAL_PROVIDER" | "REPLAY" | "TEST_DOUBLE" | "CONTROLLED_DEMONSTRATOR" | "AEGIS_DETERMINISTIC"; domain_decision: string; display_decision: "NOT_EVALUATED" | "ACCEPT" | "REJECT" | "QUARANTINE" | "BLOCKED"; evidenceRefs: string[]; message: string };
...
export type LifecyclePayload = { case: ApiCase; events: EvidenceEvent[]; graph: CaseGraph; replay?: Record<string, any> };
export type SupportLedgerStatus = { source: "BRIGHT_DATA_SUPPORT"; status: "DIAGNOSIS_PENDING" | "DIAGNOSIS_RECEIVED"; diagnosis: string | null; providerError: string | null; recommendedAction: string; evidenceReference: string; providerCalls: 0; providerMutations: 0; retries: 0 };
...
export const aegisApi = {
  ...
  getLifecycle: (caseId: string) => request(() => client.aegis.caseLifecycle.query({ caseId })) as Promise<LifecyclePayload>,
  getCase: async (caseId: string) => (await aegisApi.getLifecycle(caseId)).case,
  getEvidence: async (caseId: string) => { const payload = await aegisApi.getLifecycle(caseId); return { case_id: caseId, mode: payload.graph.mode, events: payload.events }; },
  getGraph: async (caseId: string) => (await aegisApi.getLifecycle(caseId)).graph,
  ...
  getSupportStatus: () => request(() => client.aegis.supportStatus.query()) as Promise<SupportLedgerStatus>,
};
