...
export default function CaseDetail() {
  ...
  return <AppShell><main className="case-workspace">
    ...
    {state === "ready" && caseData && <>
      ...
      {view === "overview" && <><section className="workspace-focus"><div className="focus-copy"><span className={caseData.case_id === "mission_029_real_provider" ? "source-pill source-real" : "source-pill source-local"}><Database size={13} /> {caseData.case_id === "mission_029_real_provider" ? "real provider evidence" : "persisted configuration"}</span><p className="eyebrow">Next safe action</p><h2>{caseData.case_id === "mission_029_real_provider" ? <>Provider repair stopped before a <span className="bone-emphasis">candidate existed.</span></> : <>Await evidence before <span className="bone-emphasis">making a claim.</span></>}</h2><p>{caseData.action_policy}</p>{caseData.case_id === "mission_029_real_provider" && <Link href="/cases/controlled_silent_corruption" className="outline-button">Continue with labeled controlled replay</Link>}</div><div className="focus-image">{lifecycle && <SensingField state={lifecycle} presentation="case" />}</div><div className="focus-callout"><ShieldAlert size={16} /> Bright Data collection or repair is never invoked by page load or case configuration.</div></section><section className="contract-summary"><div><span className="eyebrow">Configured protection boundary</span><h2>The case knows what to protect.</h2></div><div className="contract-columns"><div><span className="summary-label">Fields</span>{caseData.fields.map((field) => <p key={field.name}><b>{field.name}</b><span>{field.type}</span>{field.description && <small>{field.description}</small>}</p>)}</div><div><span className="summary-label">Invariants</span>{caseData.invariants.length ? caseData.invariants.map((invariant) => <p className="invariant-summary mono" key={invariant}>{invariant}</p>) : <p className="empty-copy">No invariants were supplied.</p>}<span className="summary-label case-config-label">Collection boundary</span><p className="case-config-detail"><b>Provider</b><span>Bright Data Scraper Studio</span>{caseData.collector_id ? <small>Configured collector: {caseData.collector_id}</small> : <small>No collector reference is configured.</small>}<small>No scheduler or monitoring action is connected to this case.</small></p></div></div></section></>}
